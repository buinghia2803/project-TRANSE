$(document).ready(function() {
    ...
    $('body').on('change', '[name^=result_similar_detail]', function () {
    ...
    $('body').on('click', '[type=submit]', function (e) {
        ...
        if (value == CONFIRM) {
            let isSubmit = true;
            $.each($('[name^=result_similar_detail]'), function () {
                if ($(this).val().length == 0) {
                    isSubmit = false;
                }
            });
            if (isSubmit == false) {
                productListBox.after(`<span class="notice">${errorMessageRequiredResultSimilarDetail}</span>`);
