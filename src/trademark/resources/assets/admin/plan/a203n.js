class clsEditPlanSupervisor {
    ...
    changeTypePlanName() {
        $('body').on('change', '.type_plan_name', function () {
            ...
            $(self).closest('.info_type_plan').find('.type_plan_content').html(' ')
            $(self).closest('.info_type_plan').find('.type_plan_content').val(' ')
                    ...
                    if (+typePlanId !== 8) {
                        $(self).closest('.row_plan_detail').find('.info_file').append(`
                         <div class="infor-file-item">
                            ...
                            <span class="file_info_name">${item.name}</span>
    ...
    addPlanDetail() {
        ...
        $('body').on('click', '.add_plan_detail', function () {
            ...
            let html = `
                    <tr class="row_plan_detail row_plan_detail_${count}">
                        <th>
            ...
            html += `</td>
                        ...
                        <td class="info_type_plan">
                            ...
                            <div class="parent_type_plan_description">
                                 ...
                                 <textarea hidden class="wide plan_content_edit type_plan_content" name="plan_content_edit[${keyPlan}][${count}]" style=""></textarea><br>
                            </div>
                       </td>
            ...
            $(this).closest('.parent_table_plan').find('.row_plan_detail_' + (count - 1)).after(html)
            ...
            <div class="parent_table_plan parent_plan parent_plan_${self.countPlan}" data-key-plan="${self.countPlan}">
                  ...
                  <p style="" class="parent_reasons">
                    ...
                    <input type="hidden" name="plan_reason[]" >
                        ...
                        <tr class="row_plan_detail row_plan_detail_0">
                            <th>
                              方針案<br/>
                                    (1)
                           ...
                           <td class="bg_sky">
                            ...
                            <td class="bg_sky">
                                            <span class="type_plan_doc_name white-space-pre-line"></span>
                                            <br>
                                ...
                                <td class="info_type_plan">
                                    <select class="mb10 type_plan_name type_plan_id_edit type_plan_name_${self.countPlan}_0"
                                      style="width: 350px;"
                                    ...
                                    <textarea hidden class="wide plan_content_edit type_plan_content" name="plan_content_edit[${self.countPlan}][0]"></textarea>
                                    <br>
                                </td>
        ...
        $('body').on('click', '.copy_value_to_edit_col', function () {
            ...
            $.each(typePlanDocID, function (index, item) {
                ...
                `)
                    ...
                    $(this).closest('.row_plan_detail').find('.parent_type_plan_doc_edit').append(`
                     <div class="infor-file-item infor-file-item_${index}">
                        <span class="file_info_name type_plan_doc_name_edit white-space-pre-line">${typePlanDocName[index].innerText}</span><br/>
            ...
            $.each(typePlanDetails, function (indexTypePlanDetails, item) {
                ...
                $.each(typePlanDocID, function (index, item) {
                    ...
                    if (valueItem >= 6) {
                        ...
                        $(this).closest('.row_plan_detail').find('.parent_type_plan_doc_edit').append(`
                                        <div class="infor-file-item infor-file-item_${index}">
                                            <span class="type_plan_doc_name_edit white-space-pre-line file_info_name">${typePlanDocName[index].innerText}</span><br/>
                                                <input type="hidden" class="type_plan_doc_id_edit" value="${$(item).val()}"
    ...
    checkValidateRequireDoc() {
        ...
        for (const item of typePlanDescriptionList) {
            ...
            if (valSelect == 8 && !$(item).val()) {
                if (!$(item).closest('.info_type_plan').find('.error').length) {
        ...
        for (const item of fileInfoDescriptions) {
            ...
            if (valSelectDoc && valSelectDoc != 11 && !$(item).val()) {
                if (!$(item).closest('.infor-file-item').find('.error').length) {
                    $(item).closest('.infor-file-item').find('.error').remove()
                    $(item).after(`
                        <div class="notice error">${errorMessageRequired}<div>
                    `)
                }
    ...
    onSubmit() {
        ...
        $('body').on('click', '.submit, #submit, .create_plan_reason', function (e) {
            ...
            if ( $(this)[0].id === 'submit') {
                ...
                let tablePlans = $('body').find('.table_plan')
                for (let tablePlanKey = 0; tablePlanKey < tablePlans.length; tablePlanKey++) {
                    let checksDisabled = $(tablePlans[tablePlanKey]).find('.check_disabled')
                    for (let checkDisabledKey = 0; checkDisabledKey < checksDisabled.length; checkDisabledKey++) {
                        if(!$(checksDisabled[checkDisabledKey])[0].checked) {
                            $(checksDisabled[checkDisabledKey]).closest('.parent_div_is_confirm').find('.error').remove()
                            $(checksDisabled[checkDisabledKey]).closest('.parent_is_confirm').after('<div class="error mb-2">' + errorCommonE025 + '</div>')
                        }
                    }

                    let isDecisions = $(tablePlans[tablePlanKey]).find('.is_decision')
                    for (let isDecisionsKey = 0; isDecisionsKey < isDecisions.length; isDecisionsKey++) {
                        if (+$(isDecisions[isDecisionsKey]).val() === 1) {
                            let typePlanId = $(isDecisions[isDecisionsKey]).closest('.row_plan_detail').find('.type_plan_id')
                            let typePlanDescription = $(isDecisions[isDecisionsKey]).closest('.row_plan_detail').find('.type_plan_detail_description')
                            if  (+typePlanId.val() === 0 && typePlanDescription.val().length === 0) {
                                $(tablePlans[tablePlanKey]).closest('.parent_table_plan').find('.error-table').remove()
                                $(tablePlans[tablePlanKey]).after('<div class="error error-table mb-2">' + errorValidate + '</div>')
                            }
                        } else if (+$(isDecisions[isDecisionsKey]).val() === 2){
                            let typePlanEditId = $(isDecisions[isDecisionsKey]).closest('.row_plan_detail').find('.type_plan_id_edit')
                            let typePlanDescriptionEdit = $(isDecisions[isDecisionsKey]).closest('.row_plan_detail').find('.plan_description_edit')
                            if  (+typePlanEditId.val() === 0 && typePlanDescriptionEdit.val().length === 0) {
                                $(tablePlans[tablePlanKey]).closest('.parent_table_plan').find('.error-table').remove()
                                $(tablePlans[tablePlanKey]).after('<div class="error error-table mb-2">' + errorValidate + '</div>')
                            }
                        } else {
                            $(tablePlans[tablePlanKey]).closest('.parent_table_plan').find('.error-table').remove()
                            $(tablePlans[tablePlanKey]).after('<div class="error error-table mb-2">' + errorHosinA203E001 + '</div>')
                        }
                    }
                }
