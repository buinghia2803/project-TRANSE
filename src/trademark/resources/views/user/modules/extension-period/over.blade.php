...
@section('main-content')
    ...
    <div id="contents" class="normal">
        ...
        <form action="{{ route('user.save_data_extension_period', ['id' => $trademark->id]) }}" method="post" id="form">
            ...
            @elseif($registerTrademarkRenewals->count() && $comparisonTrademarkResult->checkOverdueRegister() || !$registerTrademarkRenewals->count() && $comparisonTrademarkResult->checkOverdueRegister())
            ...
            @else
                ...
                <hr />
                <h3>{{ __('labels.u210.text_5') }}</h3>
                <ul class="eol">
                    <li class="fs12">{{ __('labels.u210.text_6') }}<span class="cost_service"></span>円</li>
                    <li class="fs12">{{ __('labels.u210.text_7') }}<span class="cost_print_fee"></span>円</li>
                </ul>
                <ul class="eol">
                    <li class="fs12">
                        {{ __('labels.u210.text_3') }}{{ $comparisonTrademarkResult->parseResponseDeadline(1) }}
                    </li>
                    <li class="fs12">
                        <strong>{{ __('labels.u210.text_4') }}{{ $comparisonTrademarkResult->parseResponseDeadline(2) }}</strong>
                    </li>
            ...
            <input type="hidden" name="from_page" value="{{ U210_OVER_02 }}">
...
@section('footerSection')
    <script>
        ...
        const checkResponseDeadline = @json($comparisonTrademarkResult->checkOverdueRegister());
        if (!countRegisterTrademarkRenewals && checkResponseDeadline || countRegisterTrademarkRenewals &&
