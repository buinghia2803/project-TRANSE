                    ...
                    @if($showColumnChoose)
                        <td class="center">
                            <input type="checkbox" class="single-checkbox productIdsChoose-{{ $item->id }}" name="m_product_ids_choose[]"
                                   data-foo="is_apply[]" data-id="{{ $item->id }}" value="{{ $item->id }}" {{ ($appTradeMarkProdsIsApplyIds && $appTradeMarkProdsIsApplyIds->contains($item->id)) || in_array($item->id, old('m_product_ids_choose', [])) ? 'checked' : '' }}/>
