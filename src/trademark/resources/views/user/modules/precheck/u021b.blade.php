...
@section('main-content')
    ...
    <div id="contents" class="normal">
        <h2>
            {{ __('labels.precheck.report.title') }}
        </h2>
        ...
        <form id="form" action="{{ route('user.precheck.post-application-trademark') }}" method="POST">
            ...
            <div class="js-scrollable mb20 highlight">
                <table class="normal_b table_product_choose">
                    <tr>
                        ...
                        <th rowspan="3" class="bg_green">{{ __('labels.precheck.table_precheck.th5') }}<br /><label><input type="checkbox" checked class="all-checkbox" />{{ __('labels.precheck.table_precheck.th5_1') }}</label></th>
                    </tr>
                    <tr>
                    ...
                    @foreach ($getProductOfDistinction as $distinction => $products)
                        @foreach ($products as $key => $item)
                            ...
                            <tr class="product-item">
                                @if ($key == 0)
                                    <td rowspan="{{ $products->count() > 0 ? $products->count() : '' }}" class="bg_blue inv_blue">
                                        {{ __('labels.precheck.table_precheck.name_distinct', [
                                            'attr' => $distinction
                                        ]) }}
                                    </td>
                                @endif
                                ...
                                <td class="center history-simple">
                                ...
                                <td class="center history-detail">
                                    @if (!empty($historyPrecheckResult) && $historyPrecheckResult->max('result_similar_detail'))
                                ...
                                <td class="center bg_green">
                                    <input type="checkbox" name="is_choice_user[]" data-foo="is_choice_user[]"
                                        {{ $item->prechecks[0]->pivot->is_apply == \App\Models\PrecheckProduct::IS_PRECHECK_PRODUCT ? 'checked' : '' }}
                    ...
                    <tr class="add-product">
                        <td colspan="11" class="right">
                            {{ __('labels.precheck.table_precheck.total_dis') }}：
                            <span class="total-dis"></span>　
                            <input type="hidden" name="sum_distintion" id="sum_distintion" value="">
                            {{ __('labels.precheck.table_precheck.total_prod') }}：
            ...
            <p>
                <input type="button" name="submitEntry" value="{{ __('labels.support_first_times.show_product_list') }}" class="btn_f" id="redirect_to_u031pass">
            </p>
            ...
            <div id="mySizeChartModal" class="ebcf_modal">
                <div class="ebcf_modal-content">
                    ...
                    <input type="submit" name="submitEntry" value="{{ __('labels.support_first_times.yes') }}" class="btn_a">
            ...
            <p><button class="btn_a" id="btn-redirect-u021c" data-route="{{ route('user.precheck.redirect-u021c')}}" style="font-size: inherit;">{{ __('labels.support_first_times.regis-by-trademark.title_h1') }}</button></p>
            ...
            <table class="normal_b mb10">
                <tr>
                    <td style="width:34em;">
                        ...
                        <span id="price_package_a">{{  CommonHelper::formatPrice($pricePackage[0][0]['base_price']) }}</span>円<br />
                        {{ __('labels.support_first_times.note_8') }}<br />
                        {{ __('labels.support_first_times.note_9') }}
                        <span id="price_product_add_pack_a">{{  CommonHelper::formatPrice($pricePackage[1][0]['base_price']) }}</span>
                ...
                <tr>
                    <td style="width:34em;">
                        ...
                        {{ __('labels.support_first_times.up_to_3_prod') }}
                        <span id="price_package_b">{{  CommonHelper::formatPrice($pricePackage[0][1]['base_price']) }}</span>円<br />
                        {{ __('labels.support_first_times.note_12') }}<br />
                        {{ __('labels.support_first_times.note_13') }}
                        <span id="price_product_add_pack_b">{{  CommonHelper::formatPrice($pricePackage[1][1]['base_price']) }}</span>
                ...
                <tr>
                    <td style="width:34em;">
                        ...
                        {{ __('labels.support_first_times.up_to_3_prod') }}
                        <span id="price_package_c">{{  CommonHelper::formatPrice($pricePackage[0][2]['base_price']) }}</span>円<br />
                        {{ __('labels.support_first_times.note_16') }}<br />
                        {{ __('labels.support_first_times.note_17') }}
                        <span id="price_product_add_pack_c">{{  CommonHelper::formatPrice($pricePackage[1][2]['base_price']) }}</span>
            ...
            <p class="eol">
                <input type="checkbox" value="2" name="period_registration" id="period_registration" {{ !empty($trademark) && !empty($trademark->appTrademark) && $trademark->appTrademark->period_registration == 2 ? 'checked': '' }} />{{ __('labels.support_first_times.regis_period.regis_10_years') }}<br />
                <span class="note">
            ...
            <ul class="footerBtn clearfix">
                <li>
                    <input type="submit" name="submitEntry" value="この内容で申込む" class="btn_e big redirect_to_common_payment" />
                </li>
            </ul>

            <ul class="footerBtn clearfix">
                <li>
                    <a href="{{  route('user.precheck.register-time-n', [
                            'id' => $id,
                            'precheck_id' => Request::get('precheck_id')
                            ]) }}" class="customer_a">{{ __('labels.precheck.go_to_the_pre-check-n') }}</a>
            ...
            <div class="estimateBox">
                ...
                <div class="estimateContents">
                    ...
                    <table class="normal_b">
                        ...
                        <tr class="d-content d_none_mailing_regis_cert">
                        ...
                        <tr>
                            <th class="right"><strong><span class="total-checkbox-checked"></span>{{ __('labels.support_first_times.cart.prod_name_small_meter') }}</strong></th>
                    ...
                    <input type="hidden" name="from_page" id="from_page" value="{{ U021B }}">
                    ...
                    <input type="hidden" name="cost_5_year_one_distintion" id="cost_5_year_one_distintion" value="{{ CommonHelper::formatPrice($periodRegistration->pof_1st_distinction_5yrs ?? 0) }}">
                    <input type="hidden" name="cost_10_year_one_distintion" id="cost_10_year_one_distintion" value="{{ CommonHelper::formatPrice($periodRegistration->pof_1st_distinction_10yrs ?? 0) }}">
                    <input type="hidden" name="total_amount" id="value_total_amount">
                    <input type="hidden" name="id" value="{{ $id }}">
                    <input type="hidden" name="precheck_id" value="{{ $precheck->id }}">
                    <p class="red mb10">※いかなる理由に関わらず、お申込み後の返金は一切ございません。</p>

                    <ul class="right list">
                        <li><input type="button" id="recalculate_money" value="{{ __('labels.support_first_times.recalculation') }}" class="btn_a" /></li>
                    </ul>

                    <ul class="right list">
                        <li><input type="submit" id="redirect_to_quote" value="{{ __('labels.support_first_times.save_display_quotation') }}" class="btn_a" /></li>
                        <input type="hidden" name="redirect_to" value="">
                    </ul>

                    <ul class="footerBtn right clearfix">
                        <li><input type="submit" name="submitEntry" value="{{ __('labels.support_first_times.apply_content') }}" class="btn_e big redirect_to_common_payment" /></li>
                    </ul>
                </div>
            </div>
            <!-- payment cart -->
...
@section('footerSection')
    <script type="text/javascript">
        ...
        const simpleOptions = @json(\App\Models\PrecheckResult::listResultSmilarSimpleOptions());
        const listRanking = @json(\App\Models\PrecheckResult::listRanking());
    </script>
    ...
    <script>
        function disableInput() {
            const form = $('form');
            form.find('input, button, textarea, select, #stop_applying, #btn-redirect-u031_edit_with_number').prop('disabled', true)
            form.find('input, button, textarea, select, #stop_applying, #btn-redirect-u031_edit_with_number').addClass('disabled')
            $('#stop_applying').off('click')
            form.find('.close-alert, #cart').prop('disabled', false);
            form.find('.close-alert, #cart').removeClass('disabled');
        }
    </script>
    @if($checkDataPrecheck || isset($appTrademark) && $appTrademark->status != $statusUnregisteredSave)
        <script>
            disableInput();
            $('[type=submit]').remove();
        </script>
