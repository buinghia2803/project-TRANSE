...
@section('main-content')
    <div id="contents" class="normal">
        ...
        <form id="form" action="{{ route('user.update.notify-procedure.post', $registerTrademark->id) }}" method="post">
            ...
            <h3>{{ __('labels.form_trademark_information.title') }}<br>
                @if(now() >= $dateUpdateBefore6Month && now() <= $dateUpdateCurrent)
                    <span class="red">
                        ...
                        {{ __('labels.u402.expired_desc') }}<br>
                    </span>
                @elseif(now() > $dateUpdateCurrent && now() <= $dateUpdateNext6Month)
                    <span class="red">
                        {{ __('labels.u402.expired_text') }}<br>
                        {{ __('labels.u402.expired_desc') }}<br>
                        <a href="{{ route('user.update.notify-procedure.overdue',  $registerTrademark->id) }}" class="no_disabled">{{ __('labels.u402.expired_link_1') }}</a>
                    </span>
            ...
            <p class="blue">
                {{ __('labels.u402.desc_1') }}<br>
                {{ __('labels.u402.desc_2') }}
            </p>
