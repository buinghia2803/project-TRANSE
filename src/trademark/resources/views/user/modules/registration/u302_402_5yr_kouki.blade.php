...
@section('main-content')
    <div id="contents" class="normal">
        ...
        <h2>{{ __('labels.u302_402_5yr_kouki.title') }}</h2>
        ...
        <form id="form" action="{{ route('user.registration.notice-latter-period.post', $registerTrademark->id) }}" method="post">
            ...
            <h3>{{ __('labels.form_trademark_information.title') }}<br>
                @if(now() >= $dateUpdateBefore6Month && now() <= $dateUpdateCurrent)
                    <span class="red">
                        {{ __('labels.u302_402_5yr_kouki.expired_text') }}<br>
                        {{ __('labels.u302_402_5yr_kouki.expired_desc') }}<br>
                    </span>
                @elseif(now() > $dateUpdateCurrent && now() <= $dateUpdateNext6Month)
                    <span class="red">
                        {{ __('labels.u302_402_5yr_kouki.expired_text') }}<br>
                        {{ __('labels.u302_402_5yr_kouki.expired_desc') }}<br>
                        <a href="{{ route('user.registration.notice-later-period.overdue',  $registerTrademark->id) }}" class="no_disabled">{{ __('labels.u302_402_5yr_kouki.expired_link_1') }}</a>
                    </span>
                @elseif(now() > $dateUpdateNext6Month)
                    <span class="red">
                        {{ __('labels.u302_402_5yr_kouki.expired_text') }}<br>
            ...
            <p class="blue">
                {{ __('labels.u302_402_5yr_kouki.desc_1') }}<br>
                {{ __('labels.u302_402_5yr_kouki.desc_2') }}
            </p>

            <dl class="w08em eol clearfix">
                <dt>{{ __('labels.u302_402_5yr_kouki.deadline_update') }}</dt>
                <dd>{{ $registerTrademark->deadlineUpdate(-5) }}</dd>
            </dl>
...
@section('script')
    ...
    @if($isBlock || now() > $dateUpdateCurrent)
