...
@section('main-content')
    ...
    <div id="contents" class="admin_wide3">
        ...
        <div class="wide clearfix">
            ...
            <form id="form" method="POST" action="{{route('admin.refusal.response-plan-re.post-supervisor', [
                    'id' => $id,
                ...
                @include('admin.modules.plans.common.common_a203', [
                    ...
                    'title' => __('labels.a203shu.title_a203n'),
                    'route' => route('admin.refusal.eval-report.create-examine.supervisor', ['id' => $dataCommon['comparisonTrademarkResult']->id, 'type' => VIEW])
                ])
                ...
                @if(count($plans) > 0)
                    @foreach($plans as $keyPlan => $plan)
                        <div class="parent_table_plan parent_plan parent_plan_{{$keyPlan}}" data-key-plan="{{$keyPlan}}">
                            ...
                            <table class="normal_b eol table_plan table_plan_{{$keyPlan}}">
                                <tr>
                                    <th></th>
                                ...
                                @foreach($plan->planDetails as $keyPlanDetail => $planDetail)
                                    <tr class="row_plan_detail row_plan_detail_{{$keyPlanDetail}}">
                                        <th>
                                        ...
                                        <td class="bg_sky parent_type_plan_detail">
                                            <div style="width: 350px;">
                                                ...
                                                <textarea hidden class="type_plan_detail_content" name="plan_content[{{$keyPlan}}][{{$keyPlanDetail}}]">
                                                    {{$planDetail->plan_content}}
                                                </textarea>
                                            </div>
                                        ...
                                        <td class="bg_blue2 parent_type_plan_detail_doc_confirm">
                                            @if(count($planDetail->planDetailDocs) > 0)
                                                @foreach($planDetail->planDetailDocs as $planDetailDocc)
                                                    ...
                                                    @else
                                                        <div style="width: 350px;" class="infor-file-item_confirm">
                                                            <span class="confirm_type_plan_doc_name white-space-pre-line">
                                                            </span>
                                                            <br/>
                                                            <br/>
                                            ...
                                            @else
                                                ...
                                                <span class="confirm_type_plan_doc_name white-space-pre-line">
                                                    ...
                                                    <br/>
                                                    <span class="confirm_doc_requirement_des"
                            ...
                            <p><a href="javascript:;"
                                  class="add_plan_detail add_plan_detail_{{$keyPlan}}" data-count="{{$keyPlan}}">+ {{__('labels.a203.add1')}}</a>
                            </p>
                            @if($keyPlan != 0)
                ...
                <ul class="footerBtn clearfix">
                    @if(!empty(request()->type) || $hasDisabled === true)
                        <li>
                            <button type="submit" class="btn_b no_disabled"
                                onclick="window.location = '{{ route('admin.refusal.response-plan.product.re-create.supervisor', [
...
@section('footerSection')
    <script>
        ...
        let hasDisabled = @JSON($hasDisabled);
    ...
    <script type="text/JavaScript" src="{{asset('admin_assets/plan/a203n.js')}}"></script>
    <script>
