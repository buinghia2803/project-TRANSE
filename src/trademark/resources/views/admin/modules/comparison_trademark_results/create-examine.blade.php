...
@section('main-content')
    <div id="contents">
        <div class="wide clearfix">
            ...
            <form id="form"
                action="{{ route('admin.refusal.eval-report.create-examine.post', $comparisonTrademarkResult->id) }}"
                method="post">
                @csrf
                    ...
                    <table class="normal_c color zebra" id="reason-table" style="min-width: 1300px;">
                        <tbody>
                            ...
                            <tr>
                                ...
                                @foreach ($reasons as $reason)
                                    <th class="bg_purple">
                                        <input type="checkbox" data-check_all data-key="{{ $loop->index }}">
                                        All
                                    </th>
                                @endforeach
                            </tr>
                            @foreach ($products as $distinctionName => $item)
                                ...
                                <tr>
                                    ...
                                    <th colspan="4" class="bg_gray right row2">
                                        {{ __('labels.create_examine.check_all') }}</th>
                                    @foreach ($reasons as $reason)
                                        <td class="bg_gray">
                                            <input type="checkbox" data-check_all_group="{{ $keyCheck }}"
                                                data-key="{{ $loop->index }}">
                                            All
                                        </td>
                                    @endforeach
                                ...
                                @foreach ($item as $product)
                                    ...
                                    <tr data-id="{{ $product->id }}"
                                        ...
                                        <td class="row2 {{ $tdClass ?? '' }}">
                                            ...
                                            <input type="hidden"
                                                name="products[{{ $product->id }}][plan_correspondence_prod_id]"
                                                value="{{ $product->plan_correspondence_prod->id }}">
                                        </td>
                                        <td class="center {{ $tdClass ?? '' }}">
                                            <span class="rank">{{ __('labels.create_examine.rank') }}：</span>
                                            <input type="hidden" class="rank_value"
                                                name="products[{{ $product->id }}][rank]" value="">
                                        </td>
                                        ...
                                        @foreach ($reasons as $reason)
                                            <td class="center middle {{ $tdClass ?? 'bg_purple' }}">
                                                <input type="checkbox"
                                                       ...
                                                       value="{{ $reason->id ?? '' }}" data-check_item="{{ $keyCheck }}"
                                                       data-key="{{ $loop->index }}"
                ...
                @if (!empty($adminUser) && $adminUser->role == ROLE_MANAGER)
                    <ul class="footerBtn clearfix">
                        <li><input type="submit" name="{{ DRAFT }}" value="{{ __('labels.save') }}"
                                class="btn_b" @if ($hasSendSeki == true) disabled @endif></li>
                        <li><input type="submit" name="{{ SUBMIT }}" value="{{ __('labels.btn_confirm') }}"
                                class="btn_c" @if ($hasSendSeki == true) disabled @endif></li>
                    </ul>
                @endif
