...
@section('main-content')
    <div id="contents">
        ...
        <div class="wide clearfix">
            ...
            <form id="form" method="POST" action="{{route('admin.refusal.eval-report.create-reason.post')}}">
                ...
                <!-- ↑↑管理画面共通【基本情報】とボタン↑↑ -->
                <input type="hidden" name="type_change" value="0">
                ...
                <h3>{{__('labels.refusal.eval_report_create_reason.human_reply_response_notice')}}</h3>

                {{-- @if($comparisonTrademarkResult->trademark->appTrademark->pack != 3) --}}
                @if(!in_array($planCorrespondence->type, [App\Models\PlanCorrespondence::TYPE_1, App\Models\PlanCorrespondence::TYPE_3]))
                    <div class="parent_response_deadline">
                        ...
                        value="{{ !empty($reasonNo->response_deadline) ?
                            \Carbon\Carbon::parse($reasonNo->response_deadline)->format('Y-m-d')
...
{{--                                <input type="hidden" name="product_type[{{$keyproductGroupByDistinction}}_{{$product->id}}]" value="{{$product->type}}">--}}
                ...
                <ul class="footerBtn clearfix">
                    ...
                    @if(isset($reasonNo) && $reasonNo->flag_role == FLAG_ROLE)
                        <li><input type="button" value="{{__('labels.refusal.eval_report_create_reason.page2')}}" class="btn_c no_disabled"
                            onclick="window.location = '{{ route('admin.refusal.eval-report.create-examine', [
                ...
                <input type="hidden" name="type_page" value="{{A201B}}">
            </form>
        </div><!-- /contents inner -->
    </div><!-- /contents -->
@endsection
@section('footerSection')
    ...
    <script>
        ...
        const MessageE034 = '{{__('messages.general.Common_E034')}}';
