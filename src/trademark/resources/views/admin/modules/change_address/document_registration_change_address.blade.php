...
@section('main-content')
    ...
    <div id="contents">
        ...
        <div class="wide clearfix">
            <form id="form" action="{{ route('admin.registration.change-address.save.document') }}" method="POST">
                @csrf
                <h3>{{ __('labels.change_address.title_document') }}</h3>
                <p> {{ __('labels.change_address.text_18') }}</p>
                ...
                <dl class="w16em eol clearfix">
                    ...
                    <dt> {{ __('labels.change_address.text_16') }}</dt>
                    <dd> {{ __('labels.change_address.text_17') }}{{ $dataSession['representative_name'] ?? '' }}</dd>

                </dl>
                <ul class="footerBtn clearfix">
                    <li><input type="submit" value="{{ __('labels.change_address.text_19') }}" class="btn_b" />
                    </li>
                </ul>
