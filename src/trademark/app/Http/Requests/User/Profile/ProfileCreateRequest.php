...
class ProfileCreateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            'info_type_acc' => [
                Rule::in(INFO_TYPE_ACC_GROUP, INFO_TYPE_ACC_INDIVIDUAL),
                'required',
                'integer',
            ],
            'info_name' => [
            ...
            'info_postal_code' => [
                'nullable',
                'regex:/^[0-9]{1,7}$/',
                'required_if:info_nation_id,==,' . NATION_JAPAN_ID,
            ],
            'info_prefectures_id' => [
                'nullable',
                'required_if:info_nation_id,==,' . NATION_JAPAN_ID,
            ...
            'info_member_id' => [
                'required',
                'unique:users,info_member_id',
                'regex:/^(?=.*[A-Za-z])(?=.*[0-9])[A-Za-z0-9-._@]{8,30}$/',
            ],
            'password' => 'required|required_with:password_confirm|regex:/^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d]{8,16}$/|same:password_confirm',
            'password_confirm' => '',
            'info_gender' => [
                'required_if:info_type_acc,==,' . INFO_TYPE_ACC_INDIVIDUAL
            ],
            'info_birthday' => [
                'required_if:info_type_acc,==,' . INFO_TYPE_ACC_INDIVIDUAL
            ],
            'info_question' => [
                'required',
                'regex:/^[ぁ-んァ-ン一-龥ａ-ｚＡ-Ｚ０-９－・]{1,100}$/',
            ],
            'info_answer' => [
                'required'
            ],
            ...
            'contact_name_manager' => [
                ...
                'max:255',
                'string',
                'required_if:contact_type_acc,==,' . CONTACT_TYPE_ACC_GROUP,
            ],
            ...
            'contact_postal_code' => [
                'nullable',
                'regex:/^[0-9]{1,7}$/',
                'required_if:contact_nation_id,==,' . NATION_JAPAN_ID,
            ],
            ...
            'contact_email_three' => [
                ...
                'regex:/^[\s]{0,60}[a-zA-Z0-9][a-zA-Z0-9_+-\.]{0,60}@[a-zA-Z0-9-\.]{2,}(\.[a-zA-Z0-9]{2,4}){1,2}[\s]{0,60}$/',
                'same:contact_email_three_confirm',
            ],
    ...
    public function messages()
    {
        return [
            ...
            'info_gender.required_if' => __('messages.update_profile.form.Common_E025'),
            ...
            'contact_type_acc.required' => __('messages.update_profile.form.Common_E025'),
