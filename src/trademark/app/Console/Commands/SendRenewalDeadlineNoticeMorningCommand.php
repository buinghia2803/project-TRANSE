...
class SendRenewalDeadlineNoticeMorningCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'notice-renewal:morning';
    ...
    {
        // Case 1: Trademarks near first renewal deadline.
        $this->nearFirstRenewalDeadline();

        // Case 8: Past the first renewal deadline
        $this->pastFirstRenewalDeadline();

        // Case 2: Nearly the renewal deadline after a successful first renewal
        $this->nearRenewalDeadlineAfterFirstRenewal();

        // // Case 9: Past the first renewal deadline after a successful first renewal
        $this->pastFirstRenewalDeadlineAfterFirstRenewal();
    }

    /**
     * Past the first renewal deadline after a successful first renewal
     */
    protected function pastFirstRenewalDeadlineAfterFirstRenewal()
    {
        try {
            $trademarks = DB::select('
                ...
                FROM trademarks
                    ...
                    WHERE trademarks.id NOT IN (
                        ...
                        AND trademark_renewal_notices.pattern = '. TrademarkRenewalNotice::PATTERN_EARLY_EXTENSION .' AND
                        trademark_renewal_notices.`type` = '. TrademarkRenewalNotice::TYPE_3 .'
                    )
                    AND ctr.response_deadline < NOW()
                    AND DATE_SUB(DATE_SUB(DATE_ADD(ctr.response_deadline, INTERVAL 2 MONTH), INTERVAL 3 DAY), INTERVAL 12 HOUR) >= now()
            ...
            foreach ($trademarks as $key => $trademark) {
                ...
                $noticeDetailShowOnU00top = $this->noticeDetailService->create([
                    ...
                    'response_deadline' => isset($trademark->response_deadline) && $trademark->response_deadline
                        ? Carbon::createFromFormat('Y-m-d H:i:s', $trademark->response_deadline)->addMonths(2)->subDays(3)
                        : null,
                    'content' => $lastNoticeDetail->content . '期間外延長のお申込み（3位）',
                    'created_at' => now()
                ]);
                ...
                //update trademark
                Trademark::find($trademark->id)->update(['block_by' => OVER_04B]);
                ...
                $this->trademarkRenewalNoticeRepository->create([
                    ...
                    'type' => TrademarkRenewalNotice::TYPE_3,
    ...
    /**
     * Trademarks near first renewal deadline.
     */
    protected function nearFirstRenewalDeadline()
    {
        try {
            DB::beginTransaction();
            $trademarks = DB::select('
            ...
            FROM trademarks
                ...
                WHERE trademarks.id NOT IN (
                    SELECT trademark_renewal_notices.trademark_id FROM trademark_renewal_notices WHERE trademark_renewal_notices.trademark_id = trademarks.id
                    AND trademark_renewal_notices.pattern = '. TrademarkRenewalNotice::PATTERN_NEVER_RENEWED .' AND
                    trademark_renewal_notices.`type` = '. TrademarkRenewalNotice::TYPE_1 .'
                )
                AND ctr.response_deadline > NOW()
                AND DATEDIFF(ctr.response_deadline, NOW()) < 10
            ...
            foreach ($trademarks as $key => $trademark) {
                ...
                // Get Last Notice Detail
                $noticeDetailLatest = $this->noticeDetailService->findByCondition([
                    'type_acc' => NoticeDetail::TYPE_USER,
                    'target_id' => $trademark->user_id,
                    'type_page' => NoticeDetail::TYPE_PAGE_TOP,
                    'is_action' => NoticeDetail::IS_ACTION_TRUE,
                ...
                $noticeDetailShowOnU00top = $this->noticeDetailService->create(
                    [
                        ...
                        'type_acc' => NoticeDetail::TYPE_USER,
                        'target_page' => '',
                        'redirect_page' => $noticeDetailLatest->redirect_page,
                        'type_notify' => NoticeDetail::TYPE_NOTIFY_TODO,
                        'type_page' => NoticeDetail::TYPE_PAGE_TOP,
                        'is_action' => NoticeDetail::IS_ACTION_TRUE,
                        'is_answer' => NoticeDetail::IS_NOT_ANSWER,
                        'response_deadline' => isset($trademark->response_deadline) && $trademark->response_deadline
                            ? Carbon::createFromFormat('Y-m-d H:i:s', $trademark->response_deadline)->subDays(3)
                            : null,
                        'content' => $noticeDetailLatest->content . '期限日前期間延長のお申込み',
                        'created_at' => now()
                    ]
                );
                ...
                //update trademark
                Trademark::find($trademark->id)->update(['block_by' => ALERT_01]);
                ...
                $this->trademarkRenewalNoticeRepository->create([
                    ...
                    'type' => TrademarkRenewalNotice::TYPE_1,
                    'is_send_notice' => TrademarkRenewalNotice::IS_SEND_NOTICE_TRUE,
                ]);
                DB::commit();
            }
    ...
    /**
     * Past the first renewal deadline
     */
    protected function pastFirstRenewalDeadline()
    {
        try {
            DB::beginTransaction();
            $trademarks = DB::select('
            ...
            foreach ($trademarks as $key => $trademark) {
                ...
                $noticeDetailShowOnU00top = $this->noticeDetailService->create(
                    [
                        ...
                        'is_action' => NoticeDetail::IS_ACTION_TRUE,
                        'is_answer' => NoticeDetail::IS_NOT_ANSWER,
                        'response_deadline' => Carbon::createFromFormat('Y-m-d H:i:s', $trademark->response_deadline)
                                                ->addMonths(2)->subDays(3),
    ...
    /**
     * Nearly the renewal deadline after a successful first renewal
     */
    protected function nearRenewalDeadlineAfterFirstRenewal()
    {
        try {
            $trademarks = DB::select('
                SELECT
            ...
            foreach ($trademarks as $key => $trademark) {
                ...
                //create notice detail show on U-000anken_top
                $noticeDetailShowOnU00ankenTop = $this->noticeDetailService->create([
                    'notice_id' => $notice->id,
                    'target_id' => $trademark->user_id,
                    'type_acc' => NoticeDetail::TYPE_USER,
                    'target_page' => '',
                ...
                //update trademark
                Trademark::find($trademark->id)->update(['block_by' => OVER_03]);
